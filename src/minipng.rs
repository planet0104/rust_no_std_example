...
struct IdatReader<'a> {
	...
	palette: Box<Palette>,
	header: Box<ImageHeader>,
	eof: bool,
}

impl<'a> IdatReader<'a> {
	fn new(reader: &'a mut SliceReader<'a>, header: Box<ImageHeader>) -> Result<Self> {
		let mut palette = Box::new([[0, 0, 0, 255]; 256]);
...
pub fn decode_png_header(bytes: &[u8]) -> Result<Box<ImageHeader>> {
...
fn read_dynamic_huffman_dictionary(reader: &mut BitReader) -> Result<(Box<HuffmanTable>, Box<HuffmanTable>)> {
	...
	Ok((
		Box::new(HuffmanTable::from_code_lengths(literal_length_code_lengths)),
		Box::new(HuffmanTable::from_code_lengths(distance_code_lengths)),
	))
}

fn get_fixed_huffman_dictionaries() -> (Box<HuffmanTable>, Box<HuffmanTable>) {
	let mut lit = Box::new(HuffmanTable::default());
	let mut dist = Box::new(HuffmanTable::default());
	for i in 0..=143 {
...
fn read_image(reader: IdatReader, writer: &mut DecompressedDataWriter) -> Result<Box<Palette>> {
	let mut reader = Box::new(BitReader::from(reader));
	// zlib header
...
/// will be returned.
pub fn decode_png<'a>(bytes: &[u8], buf: &'a mut [u8]) -> Result<Box<ImageData<'a>>> {
	...
	let mut reader = Box::new(SliceReader::from(bytes));
	let mut writer = Box::new(DecompressedDataWriter::from(buf));
	let mut palette = read_image(IdatReader::new(&mut reader, header.clone())?, &mut writer)?;
